...
###
# 
# Title: snakemake_metaphlan_cohort_pe.sh
# Date: 2024.11.12
# Author: Vi Varga
#
# Description: 
# This script will run MetaPhlAn per cohort on the non-human paired-end reads in order to 
# perform taxonomic profiling of the reads. 
# 
# Usage: 
# 	./snakemake_metaphlan_cohort_pe.sh threads
# 	OR
# 	bash snakemake_metaphlan_cohort_pe.sh threads
# 
...
ls results/DataNonHuman/BBNorm_Reads/*/*.1.fq | while read file; do
	...
	# now run MetaPhlAn
	apptainer exec workflow/containers/env-metaphlan.sif metaphlan \
	$file,results/DataNonHuman/BBNorm_Reads/${parentname}/${file_base_id}_norm.2.fq \
	--input_type fastq --bowtie2db /opt/conda/envs/env-metaphlan/lib/python3.7/site-packages/metaphlan/metaphlan_databases/ \
	--nproc $thread_count --bowtie2out results/Taxonomy/PerCohort/${parentname}/${file_base_id}_metagenome.bowtie2.bz2 \
