...
###
# 
# Title: snakemake_antismash_cohort_parallel.sh
# Date: 2025.04.28
# Author: Vi Varga
#
# Description: 
...
# 
# Usage: 
# 	./snakemake_antismash_cohort_parallel.sh threads
# 	OR
# 	bash snakemake_antismash_cohort_parallel.sh threads
# 
...
### Running AntiSMASH
# first, create input file with file information
ls results/MAGs/PerCohort/*/*_minContig1500.*.fa | while read file; do
	...
	printf "%s\t%s\t%s\n" "$file" "$file_base2" "$parentname" >> "results/BGCs/AntiSMASH/PerCohort/GNU_parallel_files.txt";
done;
...
--output-dir results/BGCs/AntiSMASH/PerCohort/{3}/{2} \
--fullhmmer --pfam2go {1}" ::::: results/BGCs/AntiSMASH/PerCohort/GNU_parallel_files.txt
